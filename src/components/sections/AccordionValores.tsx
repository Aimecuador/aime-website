    ...
    <Accordion type="multiple" className="w-full">
      {valoresYPrincipios.map((valor) => {
        ...
        return (
          ...
          >
            <AccordionTrigger className="group bg-muted/50 px-6 py-4 hover:bg-muted hover:no-underline">
              <div className="flex items-center gap-4">
                <div className="text-primary transition-colors duration-300 group-hover:text-primary/80">
                  <Icon className="h-10 w-10" />
                </div>
                <h3 className="text-left text-lg font-semibold transition-colors duration-300 group-hover:text-primary">
                  {valor.title}
                </h3>
              </div>
            </AccordionTrigger>
            <AccordionContent className="px-6 py-4 text-base text-gray-700">
